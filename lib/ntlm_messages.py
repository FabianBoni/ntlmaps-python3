...
class record:
    ...
    def create_record_info(self, offset):
        ...
        # Ensure all parts are bytes for concatenation
        if isinstance(len1, str):
            len1 = len1.encode('latin-1')
        if isinstance(len2, str):
            len2 = len2.encode('latin-1')
        if isinstance(data_off, str):
            data_off = data_off.encode('latin-1')
        self.record_info = len1 + len2 + data_off + b'\000\000'
...
def create_message1(environment_dict):
    ...
    if isinstance(msg1, str):
        msg1 = msg1.encode('latin1')
    msg1 = base64.encodebytes(msg1).decode('ascii')
    msg1 = msg1.replace('\n', '').replace('\r', '')
    return msg1
...
def create_message3(nonce, environment_dict):
    ...
    # Ensure all data parts are bytes before concatenation
    def ensure_bytes(data):
        if isinstance(data, str):
            return data.encode('latin-1')
        return data if isinstance(data, bytes) else bytes(data)
    
    data_part = (ensure_bytes(domain_rec.data) + ensure_bytes(user_rec.data) + 
                ensure_bytes(host_rec.data) + ensure_bytes(lm_rec.data) + ensure_bytes(nt_rec.data))
    ...
    head_bytes = ensure_bytes(head)
    flags_bytes = ensure_bytes(flags)
    
    m3 = (head_bytes + lm_rec.record_info + nt_rec.record_info + domain_rec.record_info + 
          user_rec.record_info + host_rec.record_info)

    # Experimental feature !!!
    if ed['NTLM_MODE'] == 0:
        m3 = m3 + additional_rec.record_info + flags_bytes

    m3 = m3 + data_part

    # Experimental feature !!!
    if ed['NTLM_MODE'] == 0:
        m3 = m3 + ensure_bytes(additional_rec.data)

    # base64 encode
    if isinstance(m3, str):
        m3 = m3.encode('latin1')
    m3 = base64.encodebytes(m3).decode('ascii')
    m3 = m3.replace('\n', '').replace('\r', '')
    return m3
