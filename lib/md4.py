...
def int_array2str(array):
    # In Python 3, build bytes then decode as latin1 for compatibility
    return bytes([int(i) & 0xFF for i in array]).decode('latin1')
...
class MD4:
    ...
    def update(self, str):
        ...
        buf = []
        # In Python 3, if str is bytes, iterating gives integers directly
        # If str is a string, iterating gives characters that need ord()
        if isinstance(str, bytes):
            for i in str: 
                buf.append(i)  # i is already an integer
        else:
            for i in str: 
                buf.append(ord(i))  # i is a character, need ord()
        ilen = U32(len(buf))
        #print (ilen)
    ...
    def digest(self):
        ...
        oldlen1 = temp.len1
        oldlen2 = temp.len2
        if (56 <= int(temp.count)):
            padlen = U32(56 - int(temp.count) + 64)
        else:
            padlen = U32(56 - int(temp.count))
        #print(int(padlen))
