...
import socket, _thread as thread, select, time
...
class proxy_HTTP_Client:
    ...
    def run(self):
        ...
        while(not self.stop_request):
            ...
            if self.config['DEBUG']['SCR_DEBUG']: print('\b.', end='', flush=True)
    ...
    def fix_client_header(self):
        ""
        # Handle NTLM_TO_BASIC translation first
        if self.config['NTLM_AUTH']['NTLM_TO_BASIC']:
            basic_auth_values = self.client_head_obj.get_param_values('Proxy-Authorization')
            for auth_value in basic_auth_values:
                if auth_value.lower().startswith('basic '):
                    self.logger.log('*** NTLM_TO_BASIC: Found Basic auth from client, extracting credentials...\n')
                    # Extract credentials using the same method as ntlm_auth
                    ...
                    if len(parts) >= 2:
                        ...
                        try:
                            ...
                            if isinstance(b64, str):
                                ...
                                b64_bytes = b64
                            cred = base64.decodebytes(b64_bytes).decode('utf-8')
                            user, password = cred.split(':', 1)
                            
                            # Parse username - check if it contains domain
                            if '@' in user:
                                username, domain = user.split('@', 1)
                                self.logger.log('*** NTLM_TO_BASIC: Extracted user: %s, domain: %s\n' % (username, domain))
                                # Update config with extracted credentials
                                self.config['NTLM_AUTH']['USER'] = username
                                self.config['NTLM_AUTH']['NT_DOMAIN'] = domain.upper()
                            else:
                                self.logger.log('*** NTLM_TO_BASIC: Extracted user: %s (no domain specified)\n' % user)
                                # Update config with extracted credentials
                                self.config['NTLM_AUTH']['USER'] = user
                            
                            self.config['NTLM_AUTH']['PASSWORD'] = password
                            # Recalculate hashed passwords
                            import ntlm_procs
                            self.config['NTLM_AUTH']['LM_HASHED_PW'] = ntlm_procs.create_LM_hashed_password(password)
                            self.config['NTLM_AUTH']['NT_HASHED_PW'] = ntlm_procs.create_NT_hashed_password(password)
                            # Remove the Basic auth header so NTLM can take over
                            self.client_head_obj.del_param('Proxy-Authorization')
                            # Temporarily disable NTLM_TO_BASIC translation for this connection
                            # so it does pure NTLM instead of trying to translate back to Basic
                            self.config['NTLM_AUTH']['NTLM_TO_BASIC'] = 0
                            self.logger.log('*** NTLM_TO_BASIC: Removed Basic auth header, disabled translation, will use pure NTLM.\n')
                            break
                        except Exception as e:
                            self.logger.log('*** NTLM_TO_BASIC: Error extracting credentials: %s\n' % str(e))
    ...
    def run_rserver_loop(self):
        ...
        if res[0]:
            try:
                ...
                if socket_data:
                    socket_data = socket_data.decode('latin-1', errors='replace')
            except:
                socket_data = ''
        ...
        # Ensure socket_data is always a string before concatenation
        if isinstance(socket_data, bytes):
            socket_data = socket_data.decode('latin-1', errors='replace')
    ...
    def send_rserver_data(self):
        ...
        if self.rserver_buffer and (not self.rserver_data_sent):
            ...
            try:
                self.client_socket.send(data.encode('latin-1'))
    ...
    def send_client_data(self):
        ...
        if self.client_buffer and (not self.client_data_sent):
            ...
            try:
                self.rserver_socket.send(data.encode('latin-1'))
